...
scripts/devbox up --profile prod --with-ios-install --with-ios-clean-install --ios-configuration Release
...
- `--profile prod`는 `mingle-app/.env.local` 또는 `mingle-app/.env`에서 먼저 찾고,
  `RN_WEB_APP_BASE_URL`/`MINGLE_WEB_APP_BASE_URL`/`NEXT_PUBLIC_SITE_URL`,
  `RN_DEFAULT_WS_URL`/`MINGLE_DEFAULT_WS_URL`/`NEXT_PUBLIC_WS_URL`를 우선 사용해
  실서버 URL을 주입하며, `--vault-app-path`가 지정된 경우 Vault 경로도 fallback으로 읽습니다.
